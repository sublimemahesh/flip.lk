...
//    });
    ...
    $('#find-member-xs').keyup(function (e) {
        if (e.which != 38) {
            if (e.which != 40) {
                if (e.which != 13) {
                    var keyword = $(this).val();
                    if (keyword == '') {
                        $('#name-list-append-xs').empty();
                    }
                    $.ajax({
                        type: 'POST',
                        url: 'post-and-get/ajax/member.php',
...
//                                        html += '<span class="chat-message-item">4 Friends in Common</span>';
                            ...
                            $('#name-list-append-xs').empty();
                            $('#name-list-append-xs').append(html);
                        }
                    });
                }
            }
    ...
    $('#name-list-append-xs').on('click', '.member', function () {

        var memberId = this.id;
        var member = $(this).attr('data-value');
        $('#member-id').val(memberId.replace("c", ""));
        $('#find-member-xs').val(member);
        $('#name-list-append-xs').empty();
        $('#find-member-xs').change(function () {
            $('#member-id').val("");
        });
    });
//    $('#name-list-append-xs').on('mouseover', '.member', function () {
//        var memberId = this.id;
//        var member = $(this).attr('data-value');
//        $('#member-id').val(memberId.replace("c", ""));
//        $('#find-member-xs').val(member);
...
//    });
    $('#find-member-xs').keyup(function (e) {
        ...
        if (e.keyCode == 40) {
            ...
            var memberId = selected.replace("c", "");
            $('#member-id').val(memberId);
            $('#find-member-xs').attr('attempt', 1);
            var membername = $('div .selected').attr('data-value');
            $('#find-member-xs').val(membername);
            $('#name-list-append-xs').empty();
            window.location.replace('profile.php?id=' + memberId);
            $('#find-member-xs').change(function (e) {
                $('#find-member-xs').attr('attempt', 0);
            });
        }
    });
    $('#find-member-xs').change(function () {
